...
if ($_POST['registro'] == "edit") {
  ...
  try {
    if ($_FILES['archivo_imagen']['size'] > 0) {
      ...
      $stmt->bind_param('issiisi', $categoria, $subcategoria, $descripcion, $ordenamiento, $activado, $imagen_url, $id_registro1);
    } else {
      $stmt = $conn->prepare("UPDATE subcategoria SET sidcategoria=?, subcategoria=?, sdescripcion=?, sorden=?,sactivo=? WHERE idsubcategoria = ?");
      $stmt->bind_param('issiii', $categoria, $subcategoria, $descripcion, $ordenamiento, $activado, $id_registro1);
    }
    $stmt->execute();
...
if ($_POST['registro'] == "eliminar") {
  ...
  try {
        $stmt = $conn->prepare("DELETE FROM `catsubcategoria` WHERE id_catSubCategoria = ?");
